...
public class BankAccountRepositoryimpl implements BankAccountRepository {
	...
	public double getBalance(long accountId) {
		double balance= jdbcTemplate.queryForObject("SELECT accountBalance FROM bankAccounts WHERE account_id= ?", 
	...
	public boolean updateBalance(long accountId, double newBalance) {
		int count = jdbcTemplate.update("UPDATE bankAccounts SET accountBalance=? WHERE account_id=?", new Object[] {newBalance,accountId});
			return count !=0;
	}
	...
	public BankAccount findBankAccountById(long accountId) {
		return jdbcTemplate.queryForObject("SELECT * FROM bankAccounts WHERE account_id=?", new Object[] {accountId}, new BankAccountRowMapper());
	...
	public BankAccount updateBankAccount(BankAccount account) {
		int count = jdbcTemplate.update("UPDATE bankAccounts SET accountHolderName = ?, aaccountType=? WHERE account_id=?", new Object[] {account.getAccountHolderName(), account.getAccountType(),account.getAccountId()});
